...
from src.router import RouteDecision, decide_route
from src.reflection import should_regenerate
...
class RAGPipeline:
    ...
    def _get_strict_chain(self):
        """反思阶段重写：同一 LLM，更强约束。"""
        if self._strict_chain is None:
            strict_template = ChatPromptTemplate.from_template(
                """你是一个专业的个人面试助手。你必须仅根据下列上下文作答，禁止编造或推断上下文未出现的经历与数据。
...
以下是初稿回答（其中若有违背上下文之处必须纠正；若初稿整体不可用请忽略初稿仅依据上下文作答）：
{draft}

请输出修订后的最终回答。若上下文不足以回答，必须明确回复「根据当前知识库内容，无法回答该问题」。

修订后的回答："""
            )
            self._strict_chain = strict_template | self.llm | StrOutputParser()
        return self._strict_chain

    @staticmethod
    def _llm_invoke_text(llm, prompt: str) -> str:
...
你的回答："""
    ...
    def _rewrite_for_retrieval_query(self, question: str) -> str:
        """慢路径：生成简短检索用语。"""
        prompt = (
            "将下列面试相关问题改写为一条简短的中文检索查询（10—40 字），用于向量检索。"
            "只输出检索用语本身，不要解释或标点包裹。\n"
            f"问题：{question.strip()}"
        )
        try:
            text = self._llm_invoke_text(self.llm, prompt).strip()
            text = text.strip("\"'“”")
            return text if len(text) >= 4 else question.strip()
        except Exception:
    ...
    def _merge_doc_results(
        ...
        b: List[Tuple[Any, Any]],
    ) -> List[Tuple[Any, Any]]:
        merged: Dict[tuple, Tuple[Any, Any]] = {}
    ...
    def _retrieve_prioritized(
        ...
        """
        相似度检索 → 可选慢路径合并 → 重排 → 阈值 → 优先级 → 来源策略。
        返回 (prioritized_results, source_policy)。
        """
        k = config.vector_store.recall_k if config.vector_store.enable_reranker else config.vector_store.search_k
        raw_results = self.vector_store_manager.similarity_search(query=effective_question, k=k)

        if route_name == "slow" and config.agent.slow_path_extra_retrieval:
            aug = self._rewrite_for_retrieval_query(question)
            if aug and aug != effective_question.strip():
                raw2 = self.vector_store_manager.similarity_search(query=aug, k=k)
                raw_results = self._merge_doc_results(raw_results, raw2)
                logger.info("慢路径补充检索已合并，改写查询: %s", aug[:80])
        ...
        return prioritized_results, source_policy

    def _resolve_route(self, question: str, route_override: Optional[str]) -> RouteDecision:
        if route_override in ("fast", "slow"):
            return RouteDecision(route=route_override, reason="client_override", confidence=1.0)
        return decide_route(
            question=question,
            invoke_llm=lambda p: self._llm_invoke_text(self.llm, p),
            route_mode=config.agent.route_mode,
            llm_confidence_threshold=config.agent.llm_router_confidence_threshold,
            agent_enabled=config.agent.enable,
        )
    ...
    def query(
        ...
        try:
            ...
            route = self._resolve_route(question, route_override)

            prioritized_results, _source_policy = self._retrieve_prioritized(
                question, effective_question, route.route
            )
            ...
            if (
                config.reflection.enable
                and config.reflection.max_regenerate_rounds >= 1
                and should_regenerate(
                    invoke_llm=lambda p: self._llm_invoke_text(self.llm, p),
                    answer=answer,
                    context=context,
                    mode=config.reflection.mode,
                    min_overlap=config.reflection.min_answer_context_overlap,
                )
            ):
                chain = self._get_strict_chain()
                answer = chain.invoke(
                    {
                        "context": context,
                        "question": effective_question,
                ...
                reflection_meta["reflection_applied"] = True
            ...
            result.update(reflection_meta)

            logger.info(f"查询完成, 找到 {len(relevant_docs)} 个相关文档, route={route.route}")
            return result
    ...
    def stream_query(
        ...
        try:
            effective_question = self._build_effective_question(question, session_context)
            route = self._resolve_route(question, route_override)
            prioritized_results, _ = self._retrieve_prioritized(
                question, effective_question, route.route
            )
