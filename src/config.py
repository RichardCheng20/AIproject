...
class AgentConfig(BaseModel):
    ...
    Attributes:
        enable (bool): 是否启用 Agent 层（关闭则始终走原单次检索）
        route_mode (str): rule_only | llm | hybrid
        slow_path_extra_retrieval (bool): 慢路径是否追加一次改写检索并合并
        llm_router_confidence_threshold (float): hybrid 下 LLM 判慢路径的置信度下限
    """
    enable: bool = False
    route_mode: str = "hybrid"
    slow_path_extra_retrieval: bool = True
    llm_router_confidence_threshold: float = 0.65


class ReflectionConfig(BaseModel):
    """
    生成后反思：校验答案与上下文一致性，必要时单次重写

    Attributes:
        enable (bool): 是否启用（仅对非流式 query() 生效）
        max_regenerate_rounds (int): 最大重写轮数，Phase1 固定为 1
        mode (str): heuristic | llm | hybrid
        min_answer_context_overlap (float): 启发式：答案与上下文词重合度下限，低于则倾向重写
    """
    enable: bool = False
    max_regenerate_rounds: int = 1
    mode: str = "hybrid"
    min_answer_context_overlap: float = 0.08
...
class ConfigManager:
    ...
    def load_config(self) -> SystemConfig:
        ...
        if os.getenv("AGENT_ENABLE"):
            system_config.agent.enable = os.getenv("AGENT_ENABLE", "false").lower() == "true"
        if os.getenv("AGENT_ROUTE_MODE"):
            system_config.agent.route_mode = os.getenv("AGENT_ROUTE_MODE", system_config.agent.route_mode)
        if os.getenv("REFLECTION_ENABLE"):
            system_config.reflection.enable = os.getenv("REFLECTION_ENABLE", "false").lower() == "true"
