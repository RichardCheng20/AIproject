...
class QueryRequest(BaseModel):
    ...
    Attributes:
        ...
        route_override (Optional[str]): 前端覆盖检索路径：fast=快思考，slow=慢思考，未传则自动路由
    """
    question: str = Field(..., min_length=1, max_length=1000)
    session_id: Optional[str] = None
    stream: bool = False
    route_override: Optional[Literal["fast", "slow"]] = None
...
class QueryResponse(BaseModel):
    ...
    Attributes:
        ...
        route (Optional[str]): fast / slow，Phase1 路由
        route_reason (Optional[str]): 路由原因摘要
        reflection_applied (Optional[bool]): 是否经过反思重写
    """
    question: str
    answer: str
...
async def query_knowledge_base(request: QueryRequest):
    ...
    for k in ("route", "route_reason", "reflection_applied"):
        if k not in result:
            result[k] = None
...
async def get_available_models():
    ...
    return {
        ...
        "agent_enable": config.agent.enable,
        "agent_route_mode": config.agent.route_mode,
        "reflection_enable": config.reflection.enable,
        "reflection_mode": config.reflection.mode,
    }
