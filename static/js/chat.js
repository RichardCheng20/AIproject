...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function buildQueryBody(question) {
        const body = {
            question: question,
            session_id: sessionId
        };
        const ro = routeModeSelect && routeModeSelect.value;
        if (ro === 'fast' || ro === 'slow') {
            body.route_override = ro;
        }
        return body;
    }
    ...
    function sendNormalRequest(question) {
        ...
        fetch('/api/query', {
            ...
            body: JSON.stringify(buildQueryBody(question))
        })
        .then(response => response.json())
        .then(data => {
            hideTypingIndicator();

            if (data.status === 'success' || data.status === 'insufficient_context') {
